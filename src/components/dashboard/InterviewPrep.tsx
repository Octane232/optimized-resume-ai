...
.from(‘interview_sessions’).select(’*’).order(‘completed_at’, { ascending: false });
if (!sessionData) return;
const full = await Promise.all(sessionData.map(async (s) => {
const { data: answerData } = await supabase.from(‘interview_answers’).select(’*’)
.eq(‘session_id’, s.id).order(‘created_at’, { ascending: true });
return {
id: s.id, position: s.position,
overallScore: Number(s.overall_score), date: s.completed_at,
answers: (answerData || []).map(a => ({
question: a.question, answer: a.answer,
feedback: { score: Number(a.score), …(typeof a.feedback === ‘object’ && a.feedback ? a.feedback : {}) },
})),
};
}));
setSessions(full);
...
if (!position.trim()) { toast({ title: ‘Enter a role first’, variant: ‘destructive’ }); return; }
const credited = await spendCredit(‘interview_prep’, `Mock interview: ${position}`);
if (!credited) { toast({ title: ‘No credits’, description: ‘You need 1 credit to start.’, variant: ‘destructive’ }); return; }
...
setQuestions(qs); setCurrentQ(0); setAnswers([]);
setCurrentFeedback(null); setUserAnswer(’’); setStage(‘question’);
} catch {
setQuestions(getFallback(position)); setCurrentQ(0); setAnswers([]);
setCurrentFeedback(null); setUserAnswer(’’); setStage(‘question’);
...
const fb: Feedback = {
score: data?.score || 5, feedback: data?.feedback || ‘Good attempt.’,
strengths: data?.strengths || [], improvements: data?.improvements || [],
};
const newAnswer: Answer = { question: questions[currentQ], answer: userAnswer, feedback: fb };
const updated = […answers, newAnswer];
setAnswers(updated); setCurrentFeedback(fb); setStage(‘feedback’); setUserAnswer(’’);
...
if (user) {
const { data: saved } = await supabase.from('interview_sessions')
.insert({ user_id: user.id, position, overall_score: overall }).select().single();
if (saved) {
await supabase.from('interview_answers').insert(
updated.map(a => ({
session_id: saved.id, question: a.question, answer: a.answer, score: a.feedback.score,
feedback: { feedback: a.feedback.feedback, strengths: a.feedback.strengths, improvements: a.feedback.improvements },
}))
);
await loadHistory();
...
} catch { toast({ title: 'Error', description: 'Could not get AI feedback.', variant: 'destructive' }); }
...
const nextQuestion = () => {
if (currentQ < questions.length - 1) { setCurrentQ(p => p + 1); setCurrentFeedback(null); setStage(‘question’); }
else { setStage(‘results’); }
};

const reset = () => {
setStage(‘setup’); setPosition(’’); setCompany(’’);
setQuestions([]); setCurrentQ(0); setAnswers([]);
setCurrentFeedback(null); setUserAnswer(’’);
};

const startLive = () => {
if (!livePosition.trim()) { toast({ title: ‘Enter your role first’, variant: ‘destructive’ }); return; }
setLiveEntries([]); setLiveActive(true);
};
...
setLiveQuestion(’’); setLiveLoading(true);
...
<div>
<label className="text-xs font-semibold text-foreground/80 uppercase tracking-wider mb-1.5 block">Target Role <span className="text-destructive">*</span></label>
<Input placeholder="e.g. Senior Backend Engineer, Product Manager"
...
<Input placeholder="e.g. Stripe, Google, early-stage startup"
value={company} onChange={e => setCompany(e.target.value)}
className="h-11 bg-muted/30 border-border/50" />
</div>
</div>
<Button onClick={generateQuestions} disabled={loadingQuestions || !position.trim() || balance <= 0}
size="lg" className="w-full h-12 font-semibold gap-2">
{loadingQuestions
? <><Loader2 className="w-4 h-4 animate-spin" />Generating questions…</>
: <><Sparkles className="w-4 h-4" />Start Mock Interview
<span className="ml-auto flex items-center gap-1 text-xs opacity-60 font-normal"><Coins className="w-3 h-3" />1 credit</span></>}
</Button>
{balance <= 0 && (
<p className="text-xs text-center text-destructive">No credits.{' '}
<button onClick={() => setActiveTab?.('billing')} className="underline font-semibold">Get more →</button>
</p>
)}
</CardContent>
</Card>
...
{[{ label: 'Sessions', value: sessions.length }, { label: 'Questions', value: sessions.reduce((s, sess) => s + sess.answers.length, 0) }].map((stat, i) => (
...
{['AI generates 5 role-specific questions', 'Answer each in your own words', 'Get instant score + feedback', 'Review what to improve'].map((s, i) => (
...
{stage === 'question' && (
<div className="space-y-3">
<Textarea placeholder="Type your answer. Use a real example from your experience. Vague answers score low."
value={userAnswer} onChange={e => setUserAnswer(e.target.value)}
className="min-h-[150px] bg-muted/20 border-border/50 resize-y text-sm" disabled={loadingFeedback} />
<div className="flex items-center justify-between">
<span className="text-xs text-muted-foreground">{wordCount} words <span className="opacity-50">— aim for 100–200</span></span>
<div className="flex gap-1">
{[0, 60, 120].map((t, i) => (
<div key={i} className={`h-1 w-6 rounded-full transition-colors ${wordCount > t ? 'bg-primary' : 'bg-muted'}`} />
))}
</div>
</div>
<Button onClick={submitAnswer} disabled={!userAnswer.trim() || loadingFeedback} size="lg" className="w-full h-11 font-semibold gap-2">
{loadingFeedback ? <><Loader2 className="w-4 h-4 animate-spin" />Analysing your answer…</> : <>Submit Answer <ArrowRight className="w-4 h-4" /></>}
</Button>
</div>
)}
...
{[...Array(10)].map((_, i) => (
<div key={i} className={`h-1.5 flex-1 rounded-full ${i < currentFeedback.score
...
<p className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3" />Strengths</p>
{currentFeedback.strengths.map((s, i) => (
<div key={i} className="flex items-start gap-2"><span className="text-emerald-500 text-xs shrink-0 mt-0.5">✓</span><p className="text-xs text-muted-foreground">{s}</p></div>
))}
</div>
)}
{currentFeedback.improvements?.length > 0 && (
<div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/15 space-y-2">
<p className="text-[10px] font-bold text-amber-600 uppercase tracking-wider flex items-center gap-1.5"><AlertCircle className="w-3 h-3" />Improve</p>
{currentFeedback.improvements.map((s, i) => (
<div key={i} className="flex items-start gap-2"><span className="text-amber-500 text-xs shrink-0 mt-0.5">→</span><p className="text-xs text-muted-foreground">{s}</p></div>
))}
</div>
)}
</div>
...
<Button onClick={reset} variant="outline" className="flex-1 h-11 gap-2"><RotateCcw className="w-4 h-4" />New Interview</Button>
{setActiveTab && (
<Button onClick={() => setActiveTab('resume-engine')} className="flex-1 h-11 gap-2"><TrendingUp className="w-4 h-4" />Tailor My Resume</Button>
)}
</div>
</motion.div>
)}
...
<div className="space-y-3">
<div>
<label className="text-xs font-bold text-foreground/80 uppercase tracking-wider mb-1.5 block">Role <span className="text-destructive">*</span></label>
<Input placeholder="e.g. Senior Product Manager" value={livePosition} onChange={e => setLivePosition(e.target.value)} className="h-11 bg-muted/30 border-border/50" />
</div>
<div>
<label className="text-xs font-bold text-foreground/80 uppercase tracking-wider mb-1.5 block">Company <span className="text-muted-foreground font-normal normal-case">(optional)</span></label>
<Input placeholder="e.g. Stripe" value={liveCompany} onChange={e => setLiveCompany(e.target.value)} className="h-11 bg-muted/30 border-border/50" />
</div>
</div>
<Button onClick={startLive} size="lg" className="w-full h-11 font-bold gap-2 bg-red-500 hover:bg-red-600 text-white border-0">
...
<p className="text-xs font-bold text-foreground uppercase tracking-wider mb-1">How it works</p>
{['Open this before your interview', 'Click Go Live and enter your role', 'Interviewer asks a question', 'Type it here — press Enter', 'Read the suggested answer approach', 'Speak confidently using the guide'].map((s, i) => (
<div key={i} className="flex items-start gap-2.5">
<div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center shrink-0 mt-px">
<span className="text-[9px] font-bold text-primary">{i + 1}</span>
</div>
...
<Button variant="ghost" size="sm" onClick={() => { setLiveActive(false); setLiveEntries([]); }} className="text-xs h-7">End</Button>
</div>
...
<Input placeholder="Type what the interviewer just asked…" value={liveQuestion}
onChange={e => setLiveQuestion(e.target.value)} onKeyDown={e => e.key === 'Enter' && askLive()}
className="h-11 bg-muted/30 border-border/50 flex-1" disabled={liveLoading} autoFocus />
<Button onClick={askLive} disabled={!liveQuestion.trim() || liveLoading} className="h-11 gap-2 shrink-0">
{liveLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}Get Help
</Button>
</div>
</div>
)}
...
{/* TIPS */}
{tab === 'tips' && (
<motion.div key="tips" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
...
<Badge variant="outline" className={`text-[10px] px-1.5 h-4 ${tip.priority === 'High' ? 'border-red-300/50 text-red-600 bg-red-500/5' : 'border-amber-300/50 text-amber-600 bg-amber-500/5'}`}>{tip.priority}</Badge>
<Badge variant="secondary" className="text-[10px] px-1.5 h-4 gap-0.5"><Clock className="w-2.5 h-2.5" />{tip.time}</Badge>
</div>
...
<Button onClick={() => setTab('practice')} className="gap-2"><Brain className="w-4 h-4" />Start Practicing</Button>
</CardContent>
</Card>
) : (
<div className="space-y-4">
<div className="grid grid-cols-3 gap-3">
{[{ label: 'Sessions', value: sessions.length, icon: Flame }, { label: 'Avg Score', value: `${avgScore.toFixed(1)}/10`, icon: Trophy }, { label: 'Questions', value: sessions.reduce((s, sess) => s + sess.answers.length, 0), icon: Target }].map((stat, i) => (
<Card key={i} className="border border-border/60 shadow-sm">
...
<div className="space-y-2">
{sessions.map((sess, i) => (
<motion.div key={sess.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
...
<Badge variant="outline" className={`text-xs shrink-0 ${sess.overallScore >= 8 ? 'border-emerald-500/30 text-emerald-600' : sess.overallScore >= 6 ? 'border-blue-500/30 text-blue-600' : 'border-amber-500/30 text-amber-600'}`}>
