...
serve(async (req) => {
  ...
  try {
    ...
    const { question, answer, position, company, generateOnly, liveMode } = body;

    // ---- MODE 1: Generate questions (no quota — quota was charged client-side on start)
    if (generateOnly) {
    ...
    // ---- MODE 2: Live coaching — return a suggested AI answer
    if (liveMode) {
    ...
    // ---- MODE 3: Score an answer (default)
    if (!question || !answer) return jsonResponse({ error: "question and answer are required" }, 400);
    const overQuota = await enforceQuota(auth, "interview_prep");
    if (overQuota) return overQuota;
