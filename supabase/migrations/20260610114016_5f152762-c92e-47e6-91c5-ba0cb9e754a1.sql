-- Update free tier limits to 0 (no free usage after trial ends)
UPDATE plan_feature_limits
SET monthly_limit = 0
WHERE tier = 'free';

-- Fix increment_usage to properly handle trial users (grant elite access during active trial)
...
DECLARE
  ...
  user_sub RECORD;
BEGIN
  -- Check subscription including trial status
  ...
  IF user_sub IS NULL THEN
    user_tier := 'free';
  ELSE
    -- Active trial gets elite limits
    ...
    ELSE
      -- Expired trial or inactive → free (0 limits)
      user_tier := 'free';
    END IF;
  END IF;
