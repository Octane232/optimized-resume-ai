...
const PricingCards = () => {
  ...
  const getSavings = (plan: typeof plans[0]) => {
    ...
    const m = parseInt(plan.monthlyPrice.replace('$', ''));
    const y = parseInt(plan.yearlyPrice.replace('$', ''));
    return `Save $${(m * 12) - y}`;
  };
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-center">
        <div className="bg-muted/50 rounded-full p-1 border border-border/60 flex gap-1">
          <button onClick={() => setBilling('monthly')}
            className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${billing === 'monthly' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
            Monthly
          </button>
          <button onClick={() => setBilling('yearly')}
            className={`px-5 py-2 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${billing === 'yearly' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
            Yearly
            <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border-0 text-[10px] px-1.5">Save 20%</Badge>
          </button>
        </div>
      </div>
      {/* Plan cards */}
      <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
        {plans.map((plan) => {
          ...
          return (
            <div key={plan.id} className={`relative flex flex-col p-6 rounded-2xl border-2 bg-card transition-all hover:shadow-lg ${
              isCurrent ? 'border-emerald-500 ring-2 ring-emerald-500/10'
              : plan.popular ? 'border-primary ring-2 ring-primary/10'
              : 'border-border'
            }`}>
              ...
              {isCurrent && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <Badge className="bg-emerald-500 text-white px-3 py-1 text-xs font-semibold shadow-md border-0">Your Plan</Badge>
                </div>
              )}
              ...
              <div className="text-center mb-5">
                ...
                {savings && <Badge className="mt-2 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border-0 text-xs">{savings}</Badge>}
              </div>
              <ul className="space-y-2 flex-1 mb-2">
                {plan.features.map((f, i) => (
                  <li key={i} className={`flex items-start gap-2.5 text-sm ${f.startsWith('(') ? 'pl-6 -mt-1' : ''}`}>
                    {!f.startsWith('(') && <Check className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />}
                    <span className={f.startsWith('(') ? 'text-xs text-muted-foreground/70' : 'text-foreground/80'}>{f}</span>
                  </li>
                ))}
 ...
 {plan.note}</p>
                  isCurrent ? 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/30 hover:bg-emerald-500/20 cursor-default'
